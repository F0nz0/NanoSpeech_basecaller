...
def retrieve_Is_idx_fasta(fasta_filepath, read_name, return_readname=False):
    '''
    Extract from a fasta indexed file (using faih generated by *index_fasta_newlines* function)
    a list of predicted inosines from the header line of the query read.
    '''
    ...
    with open(faih_idx_file, "r") as idx:
        ...
        for r in idx:
            if r.split("\t")[0] == read_name:
                ...
                # retrieve inosine idxs
                f = open(fasta_filepath, "rb")
                f.seek(offset, 0)
                line = f.read(length).decode("utf-8")[1:].strip().split(" ")
                if len(line) == 2:
                    rname = line[0]
                    idxs_list = [int(i) for i in line[1].split(",")]
                else:
                    rname = line[0]
                    idxs_list = [] # no inosine detected for this read
                f.close()
                break
        if return_readname:
            return rname, idxs_list
        else:
            return idxs_list
        

def retrieve_Is_idx_fastq(fastq_filepath, read_name, return_readname=False):
    '''
    Extract from a fastq indexed file (using faih generated by *index_fastq_newlines_quals* function)
    a list of predicted inosines from the header line of the query read and the quality row.
    '''
    ...
    with open(fqih_idx_file, "r") as idx:
        ...
        for r in idx:
            if r.split("\t")[0] == read_name:
                ...
                break
        if return_readname:
            return rname, idxs_list, quals_list
        else:
            return idxs_list, quals_list
...
# define inosine calculator
def inosine_detector(bam_filepath, fastx_filepath):
    '''
    Calculate the genomic positions of inosines called by NanoSpeech.
    It needs as input the filepath for the indexed bam and fasta/fastq files (files with inosines idx into the header 
    with .faih index).
    Iterate over each read into the bam file and performs the following operations:
    1) take the read name from the bam file
    2) extract inosine idx of the corresponding read/record from the fasta/fastq file.
    3) by the means of the MD tag it retrieves the genomic coordinate for each inosine and save it into an output file with 
       this format:
                    ...
                    if not r.is_unmapped:
                        ...
                        if hclip > 0:
                            ...
                            if r.cigartuples[0][0] == 5:
                                ...
                                # is on 3' (we don't need to consider to retrieve aligned inosines indexes --> set to 0)
                                hclip = 0
                        # retrieve inosine idxs from fasta or fastq file
                        if fastx_extention == ".fasta":
                            Iidxs = retrieve_Is_idx_fasta(fastx_filepath, qname)
                            quals_list = [0 for i in range(len(Iidxs))]
                        elif fastx_extention == ".fastq":
                            Iidxs, quals_list = retrieve_Is_idx_fastq(fastx_filepath, qname)
                        # correct Iidxs if read maps on reverse strand
                        if strand == "-":
                            Iidxs = [(read_len-1)-i for i in Iidxs]
                        # retrieve MD tag info and convert to a dataframe to convert fasta-inosine indexes to bam coordinates
                        md = r.get_aligned_pairs(with_seq=True)
                        md_df = pd.DataFrame(md, columns=["qpos", "rpos", "ref_base"])
                        for idx,qual in zip(Iidxs, quals_list):
                            q = md_df.query(f"qpos == {idx-hclip}")
                            if not q.empty:
                                refbase = (q.ref_base.iloc[0,])
                                rpos = q.rpos.iloc[0,]
                                if not pd.isnull(refbase):
                                    out.write(f"{region}\t{(rpos)}\t{rpos+1}\t{qname}\t{phred_score_to_symbol(qual)}\t{strand}\t{refbase}\n")

# aggregate to genome space
def aggregate_genome_space(per_read_preds_filepath, bam_filepath, save_to_tsv=False, min_qual=None):
    # load per read table
    df = pd.read_table(per_read_preds_filepath, header=None, names=["region", "start", "stop", "read_name", "qual", "strand", "ref_base"])
    # filter by quality if requested
    if min_qual:
        df = df.query(f"qual >= {min_qual}")
    # aggregate to genome space counting Inosines
    df_genome_space = df.groupby(["region", "start", "strand"]).size().to_frame(name="I_count").reset_index()
    del(df)
    ...
    df_genome_space["I_freq"] = df_genome_space["I_count"] / df_genome_space["depth"]
    if save_to_tsv:
